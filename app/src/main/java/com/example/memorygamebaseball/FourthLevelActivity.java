...
public class FourthLevelActivity extends AppCompatActivity {
    private static final long GAME_DURATION = 50000; // Duración del juego en milisegundos (50 segundos)
    private static final long TICK_INTERVAL = 1000; // Intervalo de actualización del contador en milisegundos (1 segundo)
    private static final int TOTAL_PAIRS = 8; // Número total de parejas en el juego
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageResources.add(R.drawable.andres_galarraga);
        imageResources.add(R.drawable.luis_arraez);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.contacto) {
            ...
            Intent intent = new Intent(FourthLevelActivity.this, ContactActivity.class);
    ...
    private void showSuccessDialog() {
        ...
        builder.setTitle("¡Felicidades!")
                .setMessage("Has completado el nivel.\nPuntaje: " + score + "\n¿Deseas pasar a la pantalla de resumen?")
                .setPositiveButton("Sí", (dialog, which) -> showScoreSummary())
                .setNegativeButton("No", (dialog, which) -> finish())
                .setCancelable(false)
                .show();
    }
    ...
    private void showScoreSummary() {
        ...
        if (playerName != null && !playerName.isEmpty()) {
            ...
            int level4Score = calculateScore(progressBar.getProgress());

            Intent intent = new Intent(this, ScoreSummaryActivity.class);
            intent.putExtra("JUGADOR", playerName);
            intent.putExtra("SCORE_LEVEL_1", level1Score);
            intent.putExtra("SCORE_LEVEL_2", level2Score);
            intent.putExtra("SCORE_LEVEL_3", level3Score);
            intent.putExtra("SCORE_LEVEL_4", level4Score);
            startActivity(intent);
            finish();
        }
    }
